...
| Prototype capturable turrets from sanitized visual shells | Roblox distinguishes instantaneous weapon checks from physically simulated projectiles, requires authoritative outcomes on the server, supports native raycasts for obstruction, and warns that Creator Store models can contain malicious scripts despite moderation. | Strip imported code and use a small source-controlled capture/target/attack contract; use hitscan for bullets and bounded server-simulated travel for shells and missiles while clients render capped cosmetics. | Whether capture improves route choice, whether nearest-target priority is fair across human/kaiju scale, and every capture/damage/range/timing value. |
| Give turrets a smaller rogue-kaiju target without coupling AI to final art | Roblox provides native path computation/waypoints and recommends bounded server work and authoritative combat. | Clone one Edit-visible reviewed template, use a throttled server state machine and native path computation, and expose stable tags/attributes so the visual can be replaced independently. | Whether the proxy/model reads as an enemy, whether its melee telegraph is fair, and every health, range, speed, timing, and path-recompute value. |
| Articulate the actual turret shells | Roblox documents connected parts as assemblies and provides `Motor6D.Transform` for animatable joints. | Keep an anchored root, validate the motor/assembly chain on the server, and move the existing body/gun geometry cosmetically on each client. | Whether each shell's rest direction, traverse, elevation and moving muzzle look correct on phone and remain within the frame budget. |
| Give the rogue kaiju observable but bounded decisions | Native path objects expose waypoints and forward blockage; Roblox's security guidance keeps AI, targets and damage on the server. | Use throttled perception with FOV, line of sight, hearing, memory, hysteresis, investigation, stuck recovery and a small attack/state set instead of importing an AI framework. | Whether a player can read the states, react to both attacks, exploit lost-target behaviour, and avoid target thrash or stuck movement. |
| Use factories to connect city capture, defenders, and possible MOBA lanes | Roblox's greyboxing guidance supports testing traversal, orientation, route choices, and bypasses before art; native streaming supports larger worlds; native pathfinding supports authored ground routes; server-security guidance requires authoritative objectives and combat. | Test one dense city island and one reinforcement factory with bounded ground/air routes, capture/destruction shutdown, and generic team/lane attributes before multiplying content. | Whether players understand the factory-enemy relationship, prefer capture or destruction choices, enjoy the island/open-land pacing, and whether the target phone can afford the streamed world plus vehicles. |
| Use one pointer-driven reticle for human fire and Brontide's Beam | Roblox documents custom cameras for over-the-shoulder play, `ViewportPointToRay()` for view targeting, safe-area insets for important UI, and server validation of weapon origin, target, obstruction, cadence, allegiance, and living state. | Let the client present a bounded reticle and send aim intent while the server owns the firing origin, cone/range validation, obstruction query, allegiance check, damage, and cooldown. | The shoulder offsets, reticle travel, Beam smoothing, target readability, and comfort across mouse, touch, and gamepad remain playtest hypotheses. |
| Permit only royalty-free commercial assets with legitimate underlying rights | Roblox Creator Store terms govern acquisition/use on Roblox services, while Roblox IP guidance still requires ownership or permission for the underlying content. | Require `£0`, monetised-use permission, no royalties/revenue share/future fees, and an underlying-IP check as separate gates. | Community listings can change or be inaccurate; each adopted asset still needs a recorded listing/terms review and a replaceable source boundary. |
...
24. [Detect hits](https://create.roblox.com/docs/tutorials/curriculums/gameplay-scripting/detect-hits) — choosing physically simulated or instantaneous hit checks according to gameplay requirements and validating weapon results.
25. [Weapons kit](https://create.roblox.com/docs/resources/weapons-kit) — Roblox-published reference for moving rocket/arrow shot effects, trails, hit effects, and explosive-projectile configuration; used as a pattern reference, not a runtime dependency.
26. [Raycasting](https://create.roblox.com/docs/workspace/raycasting) — native filtered obstruction and collision queries.
27. [Third-party asset vulnerabilities](https://create.roblox.com/docs/scripting/security/third-party-vulnerabilities) — Creator Store model backdoor risk, inspection, and sandboxing guidance.
28. [Creator Store](https://create.roblox.com/docs/production/creator-store) — model script disclosure, disabling scripts, technical details, and prohibited dynamic-loading/obfuscation patterns.
29. [Assemblies](https://create.roblox.com/docs/physics/assemblies) — joints, assembly roots, and the effect of anchoring one connected part.
30. [`Motor6D`](https://create.roblox.com/docs/reference/engine/classes/Motor6D) — animatable rigid connections and transform behaviour.
31. [`PathfindingService`](https://create.roblox.com/docs/reference/engine/classes/PathfindingService) — native path creation and agent configuration.
32. [Creator Store Terms](https://en.help.roblox.com/hc/en-us/articles/21308223046932-Creator-Store-Terms) — terms governing acquisition and use of Creator Store assets on Roblox services.
33. [Asset intellectual property](https://create.roblox.com/docs/marketplace/intellectual-property) — uploader ownership/permission and third-party-rights requirements.
34. [Greybox your environment](https://create.roblox.com/docs/tutorials/curriculums/environmental-art/greybox-your-environment) — layout-first traversal, orientation, route choice, bypass, scale, and playtest workflow.
35. [Instance streaming](https://create.roblox.com/docs/workspace/streaming) — large-world streaming behaviour, radii, model streaming, and pause outside the loaded area.
36. [Server-side detection](https://create.roblox.com/docs/scripting/security/server-side-detection) — server authority, validation, rate limits, and consequence design.
37. [Customize the camera](https://create.roblox.com/docs/workspace/camera) — Roblox's native third-person camera and custom over-the-shoulder camera option.
38. [On-screen UI containers](https://create.roblox.com/docs/ui/on-screen-containers) — safe-area insets and display ordering for unobscured HUD elements.
...
| H12 | Capturable automated turrets create readable territory play and four attack archetypes create distinct counterplay. | Uncoached ownership comprehension, capture attempts/interruptions, target switches, friendly-fire failures, cover use, projectile-type naming, dodges, damage taken by role, and physical-phone profile. | Both roles can capture and explain ownership; testers distinguish bullet, minigun, cannon and missile effects; at least one shell/missile is intentionally dodged; no friendly fire or unbounded projectile/effect growth occurs. | Slow or strengthen telegraphs, alter placement/target priority, simplify capture, retune profiles, replace unclear shells, or remove the mechanic before adding more turrets. |
| H13 | A small perceptual state machine makes the first rogue kaiju feel intentional without becoming unpredictable. | Unprompted state/attack recognition, reaction to swipe/lunge telegraphs, last-known-position investigation, target switches, stuck incidents, damage taken, and phone frame time. | Testers distinguish both attacks and at least three non-combat behaviours; no visible rapid retargeting or permanent stuck state occurs; the representative phone remains above the provisional `30 FPS` floor. | Reduce state count, strengthen pose/colour/audio cues, widen telegraphs, change sensing/memory, simplify routes, or lower decision/path frequency. |
| H14 | A reinforcement factory turns a dense city island into a readable territory objective and creates a useful bridge to MOBA lanes. | Source identification, first-wave timing, capture/destruction choice, contested time, vehicle route failures, projectile recognition/dodges, linked-turret transfer, bypass rate, completion time, and physical-phone frame/memory/network measurements. | An uncoached tester identifies the factory as the source, stops production, explains the ownership change, distinguishes cannon from helicopter fire, and reaches the next island with no unbounded runtime counts while the representative phone remains above the provisional `30 FPS` floor. | Strengthen cause/effect presentation, shorten the route, simplify capture, reduce defenders, revise vehicle paths, lower caps, or remove allied/MOBA expansion before multiplying factories. |
