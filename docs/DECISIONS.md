...
## 2026-08-02 — Make factory control asymmetric and aim both ranged attacks through one reticle

The first factory begins neutral and hostile to every nearby player. Only living human players inside its authored zone advance capture; kaiju presence neither captures nor contests it. Human capture transfers the factory, its active vehicles, future reinforcements, and explicitly linked turrets to the Human side. Brontide's alternative is physical demolition of the linked destructible, which permanently stops production for that round. A local world-space ring and event-driven ownership/progress label expose the otherwise server-only objective state.

Keyboard/mouse players use one bounded pointer-driven reticle with the custom third-person camera. The human blaster resolves immediately through the reticle; Brontide's Beam and visible head continue to approach that same intent with the existing smoothing. Touch and gamepad retain a centred reticle and native camera behavior. The client computes presentation and aim intent, while the server derives the true muzzle, validates role, life, cadence, finite direction, forward cone, allegiance and obstruction, and owns all damage.

Basis: Roblox's custom-camera guidance explicitly supports over-the-shoulder views, its UI guidance recommends safe-area-aware important HUD, and its client/server security guidance requires weapon origin, target, obstruction, firing cadence, team relationship and living state to be checked authoritatively. Whether neutral pressure, human-only capture, the virtual-reticle travel, shoulder offset, and Beam lag are understandable and enjoyable remains a playtest hypothesis; the first two-client and physical-device test must decide before factories are multiplied.

## 2026-08-02 — Fix desktop aim to a stable shoulder reticle

Keep the keyboard/mouse reticle fixed at a provisional `5.5%` of viewport width to the right of centre while the captured mouse controls the camera. Touch and gamepad retain a centred intent point. Remove the second mouse-delta consumer from `AimController`; the camera now owns mouse motion, and the reticle owns the exact `ViewportPointToRay()` position displayed to the player.

Human fire and Beam continue to send that ray-derived direction. Smash and Charge now send it as well; the server reduces those two physical attacks to a horizontal direction, rejects non-finite or more-than-`75°` off-forward intent, and retains authority over their hitboxes, movement, damage, cooldown, and state.

Basis: an owner keyboard/mouse playtest reported that normal aiming placed the crosshair far to the left and that attacks must converge on the visible point. Inspection found that `CameraController` and `AimController` both consumed `GetMouseDelta()` while the pointer was locked: the camera rotated and the virtual reticle accumulated the same movement toward the edge. A stable shoulder reticle matches the established captured-mouse camera model and eliminates that competing state. The `5.5%` offset is a provisional tuning value; an owner test must confirm perceived alignment for both human and kaiju scale before publication.

## 2026-08-02 — Make captured production a human reward and demolition the kaiju objective verb

Supersede neutral factory pressure and two-role turret capture. A neutral factory remains dormant until a living human completes its server-owned capture; capture starts bounded Human-owned tank and helicopter production and transfers linked turrets. Kaiju presence never advances or contests capture. Brontide instead damages turret health with Smash, Charge, or Beam and permanently stops factory production for the round by collapsing its linked structure.

Captured vehicles retain authored routes but only fire with server-confirmed line of sight. They prefer an existing visible target before switching, remember it briefly after occlusion, lead projectiles from measured travel time and target velocity, and advance past a route node after bounded lack of progress. These are small extensions to the existing server loop, not a general behaviour-tree dependency.

Beam is now a four-second server-owned session fired by right-click, `R`, gamepad, or touch. The client sends rate-limited aim intent; the server validates the living kaiju, round, cooldown, direction cone, duration and cadence, resolves fixed-rate sphere casts, and sends cosmetic endpoints. The client reuses two thick beam parts and emits capped impact particles instead of allocating a new beam every sample.

Basis: stakeholder playtests requested human capture as the strategic reward, kaiju destruction as the asymmetric alternative, dormant neutral factories, more credible vehicle behaviour, and a longer, stronger mouse-following Beam. Roblox's authority, raycast, and performance guidance supports bounded server outcomes and client-local transient effects. Four seconds, damage, health, target-memory, lead and stuck thresholds remain provisional playtest values.
