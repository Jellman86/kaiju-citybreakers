...
| Smash animation | Native R15 `Motor6D`, `TweenService`, and the existing Brontide shell | **Use provisionally** | A short procedural pose requires no uploaded animation, external rig, licence, cost, or additional remote. It affects presentation only; the server still validates and applies the hit. Replace with an original authored animation asset when the final rig exists. |
