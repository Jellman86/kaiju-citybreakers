...
- Replace the phone-failed small circular mark with an attack-scaled irregular breach: layered dark depth, displaced non-emissive rim pieces, and a strict eight-part-per-mark ceiling. Pair Smash with a short local wind-up, fast strike, impact response, and eased recovery while the server remains authoritative for the actual hit.
...
- Confirm Smash animation returns every procedural joint offset to its original pose, does not double-play on predicted confirmation, and still produces exactly one server-authoritative hit per accepted request.
